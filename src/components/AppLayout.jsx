...
html[data-theme="light"] .nav-link.active {
  background: rgba(10,191,188,0.1) !important;
  color: #0a0a0a !important;
  border-color: rgba(10,191,188,0.3) !important;
}
html[data-theme="light"] .section-label { color: rgba(0,0,0,0.38) !important; }
html[data-theme="light"] .btn-primary { background: #0ABFBC !important; color: #0a0a0a !important; }
        ...
        <div className="flex h-screen bg-jet overflow-hidden">
            ...
            >
                ...
                <div style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.05)', position: 'relative' }} ref={profileRef}>
                    ...
                    >
                        <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: '#0a1a1a', border: '1px solid rgba(10,191,188,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <span style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '11px', color: '#0ABFBC' }}>{userInitials}</span>
                        </div>
                        <AnimatePresence>
                            {!collapsed && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ overflow: 'hidden', flex: 1, minWidth: 0 }}>
                                    ...
                                    <p style={{ ...MONO, fontSize: '9px', color: isPro ? '#0ABFBC' : '#444', letterSpacing: '0.1em' }}>{isPro ? 'PRO' : 'FREE'}</p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                        {!collapsed && (
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        <span style={{ ...MONO, fontSize: '12px', color: '#0ABFBC', letterSpacing: '0.06em' }}>{time} IST</span>
                        {!isPro && (
                            <button
                                onClick={() => navigate('/app/billing')}
                                style={{
                                    ...MONO, fontSize: '10px', letterSpacing: '0.15em', padding: '7px 16px',
                                    background: '#0ABFBC', color: '#0a0a0a', border: 'none', borderRadius: '9999px',
                        ...
                        {isPro && (
                            <span style={{ ...MONO, fontSize: '10px', color: '#0ABFBC', border: '1px solid rgba(10,191,188,0.25)', padding: '5px 12px', borderRadius: '9999px', letterSpacing: '0.15em' }}>
                                PRO
                            </span>
                        )}
                    </div>
                            ...
                            <div style={{ padding: '12px', borderBottom: '1px solid rgba(255,255,255,0.05)', position: 'relative' }} ref={mobileProfileRef}>
                                ...
                                >
                                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#0a1a1a', border: '1px solid rgba(10,191,188,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                        <span style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '13px', color: '#0ABFBC' }}>{userInitials}</span>
                                    </div>
                                    <div style={{ textAlign: 'left', flex: 1 }}>
                                        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '13px', fontWeight: 600, color: '#fff' }}>{username}</p>
                                        <p style={{ ...MONO, fontSize: '9px', color: isPro ? '#0ABFBC' : '#444', letterSpacing: '0.1em' }}>{isPro ? 'PRO' : 'FREE'} · tap to manage</p>
                                    </div>
                                ...
                                <AnimatePresence>
                                    {profileOpen && (
                                        ...
                                        >
                                            <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <p style={{ ...EYEBROW, color: '#444' }}>Plan</p>
                                                <span style={{ ...MONO, fontSize: '10px', color: isPro ? '#0ABFBC' : '#555', background: isPro ? 'rgba(10,191,188,0.08)' : 'rgba(255,255,255,0.04)', border: `1px solid ${isPro ? 'rgba(10,191,188,0.2)' : 'rgba(255,255,255,0.08)'}`, padding: '2px 8px', borderRadius: '9999px' }}>
                                                    {isPro ? 'PRO' : 'FREE'}
                                                </span>
                                            </div>
